...
| Upstream's unified `items[]` tile rows, `tile-toggle`, and the click-at-own-output fix (mrzor/agentic-db 49f2598, 2191cc2, 64f51ec) | All three sit on the evolved waybar-pwetty-box `claude` tile schema (`items[]` mixed rows, `win_id`); this fork's tile checkout speaks the `sessions[]` contract, and in that contract a toggle has almost nothing to toggle. | Updating the local waybar-pwetty-box tile to the `items[]` schema — then port the three together. |
| Upstream's fleet/herd stack (session identity columns, fleet providers, herdr roster, `doctor fleet`, `gc --orphans`; mrzor/agentic-db 378e951 through 73a3c40) | ~10k lines built around herdr and background-agent colonies, neither of which runs on this desktop; the standalone good ideas inside it (pid-reuse-proof liveness, the background hook skip) were ported individually with credit. | Running herdr, or background-agent colonies becoming routine here — then adopt the stack as a whole rather than re-deriving it. |
